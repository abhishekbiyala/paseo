...
export default function HostNewWorkspaceRoute() {
  ...
  const initialRefName = typeof params.ref === "string" ? params.ref : undefined;

  if (!sourceDirectory) {
    return null;
  }

  const screenKey = `${serverId}:${sourceDirectory}:${initialRefName ?? ""}`;
